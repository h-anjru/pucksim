...
%PUCKSIM(height,speed) simulates the point cloud of a UAS-mounted Velodyne
%   PUCK(TM) VLP-16 Laser Scanner.
%   
%   This function models the VLP-16 scanner as a point moving parallel to a
%   target plane (specified in puckInt.m) at the specified flying height. 
%   The laser returns are modeled by lines with a known direction passing
%   through the scanner point and intersecting the target plane.
%   
%   The scanner is assumed to be oriented such that its +z axis is
%   coincident with the direction of travel, which has been arbitrarily 
%   chosen to be the +Y direction in object space. The scanner's y-axis is
%   coincident with the -Z direction on object space. This can be 
%   manipulated as described below.
%   
%   PUCKSIM(height,speed) outputs a text file that records the 
%   intersections of the modeled laser returns with the target plane. These
%   inputs are required. A speed of 0 (zero) will model a single rotation
%   of the scanner head.
%   
%   PUCKSIM(height,speed,'rotationRate',R) changes the rotation rate of the
%   scanner head. By default, the rotation rate is set to 10 Hz. The 
%   VLP-16 manual indicates the rotation rate can vary between 5-20 Hz 
%   (p.20).
%   
...
%   struck by lasers from all 16 channels. This profile is a "strip" of 
%   points L-m long in the direction of flight. This profile can be used to
%   represent theoretical point density of a strip for a flight over an 
%   unobstructed target plane.
%   
%   PUCKSIM(height,speed,'maxRange',M) limits the maximum range of 
%   recorded returns to M [m]. The VLP-16 records returns up to 120 m, the
%   default value for maxRange.
%
%   PUCKSIM(height,speed,'tilt',T) changes the orientation of the 
%   scanner. By default, the scanner is oriented as described above; this 
%   can also be thought of as the scanner "facing down." (More formally, 
%   this is a rotation of -90° about the scanner's x-axis.)
%  
...
%   
%   Example:
%      % Get a 5-m profile of points collected from a flying height of
%      % 50 m, forward speed of 10 m/s, and a rotation rate of 20 Hz, with
%      % the scanner inclined 15° from nadir (-75° about its x-axis), and 
%      % the aircraft crabbing at -5°. Limit range to 60 m.
%      
...
% t_cycle = [t_cycle(1:2) t_cycle(16)];

% range of azimuths at which a laser could theoretically intersect with
% the target plane is (3pi/2,pi/2). The PUCK has a maximum range of 100 
% meters, so this range of azimuiths is limited further.
...
XYZ = [zeros(1,16);p.Results.speed*t_cycle;ones(1,16)*p.Results.height];
% XYZ = [zeros(1,3);p.Results.speed*t_cycle;ones(1,3)*p.Results.height];
...
if p.Results.profile ~= false
    ...
    a_max_ = acos((p.Results.height/range_max + ...
        cos(omega_s)*sin(max(omega)))/ ...
...
str = ['%8.3f %8.3f %8.3f %11.8f %11.8f %12.9f' ...
...
%         fprintf('%6.1f%% complete...\n',100*t_cycle(1)/flight_time)
    ...
    % if azi(1) > a_range(1) || azi(3) < a_range(2)
        ...
        columns_to_remove = file_info(10,:) > range_max; % result: logical
...
function distances = puckRange(scanner_point,points)
% PUCKRANGE Solves for the slant distances between the modeled scanner 
%   positions and the modeled laser returns for the puckPointDensity
%   function.
%   
%             Call: distances = puckRange(scanner_point,points)
%   
%    scanner_point: 3 x n coordinates of points on line
%           points: 3 x n coordinates of line-plane intersection
%   
%        distances: 1 x n vector of slant distances

diff_squared = (scanner_point - points).^2;
distances = sqrt(sum(diff_squared,1));

end


function [points,varargout] = puckInt(scanner_point,azimuth,channel,os,kb)
% PUCKINT Solves for the point of intersection of a line and plane
%  in 3-space for the puckPointDensity function.
%  
...
% solve for line-plane intersections
for ii = 1:n
    d = dot((plane_point(:,ii)-scanner_point(:,ii)),plane_normal(:,ii))/...
